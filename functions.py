...
from data import get_user_ball,users
from states import get_user_name
...
async def start_command_answer(message: Message, state: FSMContext):
    if(message.text[7:].isdigit()):
        if users.get(str(message.from_user.id)):
            ...
            else: await message.answer("Siz botga tashrif buyurgansiz va referal bo'la olmaysiz!")
        else:    
            reffer_id = message.text[7:]
            users[str(message.from_user.id)] = {"reffer_id": reffer_id, "flag": False}

            await state.set_state(get_user_name.name)
            await message.answer("Menga haqiqiy ismingizni kiriting.")
    else:
        if users.get(str(message.from_user.id)):
            if users.get(str(message.from_user.id)).get("name"):
                await message.answer("Asisiy menyudasiz!", reply_markup=main_menu)
            else: 
...
async def get_user_name_answer(message: Message, state: FSMContext):
...
async def get_ref_link_answer(message: Message,state: FSMContext):
    if users.get(str(message.from_user.id)):
        ref_link = f"https://t.me/aiogram_tuto_aziz_bot?start={message.from_user.id}"
...
async def get_user_ball_answer(message: Message, state: FSMContext):
    if users.get(str(message.from_user.id)):
        ...
        await message.answer(f"Sizning balingiz: {user_ball} ball")
    else:
        users[str(message.from_user.id)] = {"reffer_id": None, "flag": False}
        # print(message.from_user.id)
        await state.set_state(get_user_name.name)
